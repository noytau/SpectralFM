#!/usr/bin/env bash
# T5/T5b — 3AE reconstruction heads trained FROM SCRATCH on the frozen Feb-25 SSL backbone.
# Full backbone (FE+LN+proj+transformer) loaded via --ckpt from the fairseq SSL checkpoint;
# everything frozen; head_fe / head_proj / head_trans random-init and ALL trained.
# Env overrides: MANIFEST, LR, WARMUP, WANDB_PROJECT.
...
MANIFEST="${MANIFEST:-${REPO}/fairseq/data/nova_data/single_channel_1k/train_mnt5.tsv}"
...
exec /mnt5/noy/miniconda3/envs/spectralfm/bin/python3 ${REPO}/code/train_reconstruction.py \
    ...
    --normalize \
    --data_dir /mnt5/noy/SpectralFM/fairseq/data/nova_data/single_channel_10k/wav --n_samples ${N_SAMPLES} \
