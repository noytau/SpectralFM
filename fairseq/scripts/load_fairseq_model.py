...
# This ensures Python can find 'examples' if running from a subdir (like scripts/)
...
def load_data2vec_model(checkpoint_path, data_path_override=None, user_dir=None, extra_code_paths=None, cpu=False):
    ...
    Args:
        ...
        data_path_override (str, optional): Overrides the data directory in the config. 
        user_dir (str, optional): Path to a fairseq user directory (plugins).
        extra_code_paths (list[str], optional): List of paths to custom code directories to add to sys.path.
        cpu (bool): If True, forces the model to load onto CPU.

    Returns:
        model (FairseqModel): The loaded model in eval mode.
        cfg (OmegaConf/Namespace): The configuration object used.
        task (FairseqTask): The task object (contains dictionaries, etc).
    """
    ...
    # If your model relies on code in /mnt5/noy/code, this makes it importable.
    if extra_code_paths:
    ...
    # This registers 'data2vec_audio' architecture so Fairseq recognizes it.
    if user_dir:
    ...
    # This allows loading a model even if the original data path has moved.
    arg_overrides = {}
...
# --- Example Usage Block (for testing directly) ---
