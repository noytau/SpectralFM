...
def run_step4(step1_output_dir: str, seed: int = 42) -> dict:
    ...
    if best_combo_X.shape[1] > 256:
        # PCA is unsupervised: fitting on all X is transductive but leaks no
        # labels, so this stays as-is (documented as transductive).
        Xp = PCA(n_components=256, random_state=seed).fit_transform(best_combo_X)
        axis3["pca_256"] = score(Xp)
    if best_combo_X.shape[1] > 64:
        # PLS IS SUPERVISED. An earlier version called
        #   PLSRegression(...).fit_transform(best_combo_X, y)
        # and cross-validated the result -- fitting the projection on every
        # label, including the test folds. That leaked, and inflated PLS by
        # +0.08-0.09 R2 at k=64 (measured), which reversed the sign of the
        # "PLS beats concat" conclusion. The projection must be fitted inside
        # the training fold only; reducers.eval_reducer does that.
        from .reducers import eval_reducer
        axis3["pls_64"] = eval_reducer("pls", 64, best_combo_X, y, seed=seed)
